...
jQuery( document ).ready(
	...
	function heaven11_resize_video(cont) {
		...
		cont.find( 'video' ).each( function() {
			...
			var video     = $self.addClass( 'heaven11_resize' ).eq( 0 );
		...
		// Resize tags 'iframe'
			cont.find( '.video_frame iframe,iframe' ).each( function() {
				var $self = jQuery( this );
				// If item now invisible
				if ( $self.hasClass( 'trx_addons_resize' ) || $self.hasClass( 'trx_addons_noresize' ) || $self.addClass( 'heaven11_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0 ) {
					return;
				}
		...
		function heaven11_init_media_elements(cont) {
			...
			if (HEAVEN11_STORAGE['use_mediaelements'] && cont.find( media_selector ).length > 0) {
				if ( window.mejs ) {
